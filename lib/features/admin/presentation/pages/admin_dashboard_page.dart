...
class _AdminDashboardPageState extends State<AdminDashboardPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      try {
        final response = await http.get(Uri.parse(ApiConfig.dresses));
    ...
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: const [
          ...
          BottomNavigationBarItem(icon: Icon(Icons.checkroom), label: 'Dresses'),
          BottomNavigationBarItem(icon: Icon(Icons.history), label: 'Try-On'),
        ],
      ),
    );
  }
          ...
          children: [
            ...
            GridView.count(
              ...
              children: [
                _buildStatCard(
                  'Total Try-Ons',
                  '${_stats['try_on_count'] ?? 0}',
                  Icons.camera_alt,
                  Colors.blue,
                ),
                _buildStatCard(
                  'Wishlist Items',
                  '${_stats['wishlist_count'] ?? 0}',
                  Icons.favorite,
                  Colors.red,
                ),
                _buildStatCard(
                ...
                Row(
                  ...
                  children: [
                    if (order['status'] == 'pending')
                      ElevatedButton(
                        onPressed: () async {
                          await LocalDatabaseService.updateOrderStatus(
                            order['order_id'],
                            'completed',
                          );
                          _loadData();
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          Text(
                            dress['isUpcoming'] == true ? 'Upcoming' : 'Available',
                            style: TextStyle(
                              color: dress['isUpcoming'] == true
                                  ? Colors.orange
                                  : Colors.green,
          ...
          child: Row(
            children: [
              ClipRRect(
                ...
                child: CachedNetworkImage(
                  imageUrl: ApiConfig.getUploadUrl(item['dress_image'] ?? ''),
                  width: 80,
                  height: 80,
                  fit: BoxFit.cover,
                  placeholder: (context, url) => Container(
              ...
              Expanded(
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                        item['dress_name'] ?? 'Unknown',
                        style: const TextStyle(fontWeight: FontWeight.bold),
                      ),
                      Text(
                        'Tried on: ${_formatDate(item['created_at'])}',
                        style: const TextStyle(fontSize: 12),
                      ),
                      Row(
                        children: [
                          ...
                          const SizedBox(width: 8),
                          Text(
                            item['is_favorite'] == 1 ? 'Favorited' : 'Not favorited',
                            style: const TextStyle(fontSize: 12),
                          ),
                        ],
                      ),
