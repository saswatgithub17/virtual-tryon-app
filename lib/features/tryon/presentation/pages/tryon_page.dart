...
class _TryOnPageState extends ConsumerState<TryOnPage>
  ...
  Widget _buildUserPhotoHeader(TryOnState state) {
    if (state.userPhoto == null) {
      return _buildNoPhotoUploadSection();
    }
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const Text('Select up to 5 dresses to try on',
                    style:
  ...
  Widget _buildNoPhotoUploadSection() {
    return Container(
      margin: const EdgeInsets.all(20),
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 8),
          const Text(
            'Take a photo or upload from gallery to start trying on dresses',
  ...
  Future<void> _captureFromCamera() async {
    try {
      ...
      final PickedImage? image = await cameraService.capturePhoto(frontCamera: true);
